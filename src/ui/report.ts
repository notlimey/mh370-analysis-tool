...
export function generateRunReport(
  ...
  const nonDefaultLines = Object.entries(config)
  ...
  return [
    ...
    summary.fuelFeasibleCount != null ? `Fuel-feasible paths: ${summary.fuelFeasibleCount}${summary.fuelFeasiblePercent != null ? ` (${summary.fuelFeasiblePercent.toFixed(0)}%)` : ""}` : undefined,
...
export function generateComparisonReport(left: SavedRun | null, right: SavedRun | null): string {
  ...
  const configDiffs = listConfigDiffs(left.config, right.config)
    .map((diff) => `${String(diff.key)}: ${diff.left} -> ${diff.right}`);
  const resultDiffs = [
    ...
    diffLine("peak", formatPeak(left.summary.peakLat, left.summary.peakLon), formatPeak(right.summary.peakLat, right.summary.peakLon)),
