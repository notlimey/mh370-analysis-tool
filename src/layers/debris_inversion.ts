...
export function renderDebrisInversionLayer(
  map: MapboxMap,
...
export function renderComparisonOverlay(
  map: MapboxMap,
  result: InversionResult,
): void {
  removeLayers(map, COMPARISON_LAYER_IDS, COMPARISON_SOURCE_ID);

  const satelliteSegment = segmentForLatBand(result.candidates, result.satellite_peak_lat - 1.0, result.satellite_peak_lat + 1.0);
  const debrisSegment = segmentForLatBand(result.candidates, result.confidence_interval_68[0], result.confidence_interval_68[1]);
  const intersectionSegment = segmentForLatBand(result.candidates, result.intersection_lat - 0.6, result.intersection_lat + 0.6);
...
export function setDebrisInversionVisible(
  map: MapboxMap,
  visible: boolean,
): void {
