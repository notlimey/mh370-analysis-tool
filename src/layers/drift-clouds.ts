...
import type { BackendBeachedParticle, BackendBeachingCloud } from "../lib/backend";
...
export function initDriftCloudsLayer(map: MapboxMap): void {
  ...
  map.addLayer({
    ...
    paint: {
      ...
      "circle-stroke-color": ["case", ["get", "selected"], "rgba(255,255,255,0.3)", "rgba(255,255,255,0.1)"],
  ...
  map.on("mouseenter", "drift-clouds-beached", (e) => {
    ...
    if (!props) return;
    beachPopup.setLngLat(e.lngLat).setHTML(`<strong>${props.coast}</strong><br>${props.days} days`).addTo(map);
  });
...
function rebuildDynamicSources(map: MapboxMap): void {
    ...
    if (cloud.still_drifting.length > 0 && isSelected) {
      ...
      traceFeatures.push({
        ...
        geometry: {
          type: "LineString",
          coordinates: [
            [cloud.origin_lon, cloud.origin_lat],
            [centLon, centLat],
          ],
        },
      });
  ...
  (map.getSource(SOURCE_ORIGINS) as GeoJSONSource | undefined)?.setData({
    type: "FeatureCollection",
    features: originFeatures,
  });
  (map.getSource(SOURCE_BEACHED) as GeoJSONSource | undefined)?.setData({
    type: "FeatureCollection",
    features: beachedFeatures,
  });
  (map.getSource(SOURCE_TRACES) as GeoJSONSource | undefined)?.setData({
    type: "FeatureCollection",
    features: traceFeatures,
  });
  (map.getSource(SOURCE_DRIFTING) as GeoJSONSource | undefined)?.setData({
    type: "FeatureCollection",
    features: driftingFeatures,
  });
}
